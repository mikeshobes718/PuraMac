...
- Smart Clean: scans user level caches, logs, the Trash, Xcode DerivedData, iOS DeviceSupport, the npm cache, hidden user caches, the Homebrew cache and Downloads files older than 30 days. Every category has a plain English description and an expandable list of its largest items so you can see exactly what would move before you agree. Shows per category sizes, lets you pick categories, shows a review sheet with exactly what will move, then moves everything to the Trash with a summary and a notification when done.
- Large Files: finds the top 50 files over 100 MB in your user folders (Desktop, Documents, Downloads, Movies, Music, Pictures plus Library caches, logs and Developer). Optional thumbnails preview media files. Select files and move them to the Trash after a confirmation.
